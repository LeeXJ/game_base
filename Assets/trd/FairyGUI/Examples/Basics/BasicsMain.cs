...
{
    public static IMeshFactory GetMeshFactory(this NGraphics graphics, Type type)
    {
        if (type == typeof(EllipseMesh))
        {
            return graphics.GetMeshFactory<EllipseMesh>();
        }
        if (type == typeof(PolygonMesh))
        {
            return graphics.GetMeshFactory<PolygonMesh>();
        }
        return null;
    }
}